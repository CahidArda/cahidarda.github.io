...
const SCN: Scn[] = [
  {
    label: "Read another tenant's data",
    target: 'marketplace',
    allow: false,
    by: 'RBAC: the retail identity has no role outside its own namespace',
  },
  {
    label: "Reach another tenant's pod",
    target: 'marketplace',
    allow: false,
    by: 'NetworkPolicy: deny-by-default blocks cross-tenant traffic',
  },
  {
    label: 'Publish to the event log',
    target: 'log',
    allow: true,
    by: 'NetworkPolicy: the shared log is the one allow-listed egress',
  },
  {
    label: 'Outgrow its resource budget',
    target: 'quota',
    allow: false,
    by: 'ResourceQuota: the namespace is capped, containing the hog',
  },
];
    ...
    <Widget title="What can a tenant do?" kicker="isolation: rbac · netpol · quota">
      ...
      >
        ...
        <line
          ...
          x2={to.x}
          y2={to.y}
          markerEnd={`url(#${s.allow ? 'ti-allow' : 'ti-deny'})`}
          style={{
            stroke: v,
            strokeWidth: 2.5,
            strokeDasharray: s.allow ? undefined : '5 4',
        ...
        {!s.allow && (
          <text
            ...
            y={(from.y + to.y) / 2 - 4}
            textAnchor="middle"
            style={{ fill: DENY, fontFamily: 'var(--font-mono)', fontSize: 16, fontWeight: 700 }}
          >
            ✕
          </text>
        )}
        ...
        <SvgNode
          ...
          sub="another tenant"
          color={v}
          state={s.target === 'marketplace' ? 'active' : 'off'}
          titleSize={11}
        />
        <SvgNode
          n={NODE.log}
        ...
        <SvgNode
          ...
          sub="its own budget"
          color={v}
          state={s.target === 'quota' ? 'active' : 'off'}
          titleSize={11}
        />
      </svg>
      ...
      <div className="mt-2 flex items-center gap-2">
        <span
          className="border-2 px-2 py-0.5 font-mono text-[0.72rem] font-semibold"
