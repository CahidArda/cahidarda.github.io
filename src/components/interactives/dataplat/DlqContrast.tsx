...
export default function DlqContrast() {
  ...
  return (
    <Widget title="Two ways to dead-letter" kicker={isLog ? 'the consumer decides' : 'the broker decides'}>
      <div className="mb-3 flex flex-wrap gap-2">
        {TABS.map((t) => (
          <button key={t.key} type="button" onClick={() => setSel(t.key)} aria-pressed={tab === t.key} {...bindTab(t.key)}
            className="border-2 px-2.5 py-1 font-mono text-[0.72rem] tracking-wide transition-colors"
            style={{ borderColor: tab === t.key ? 'var(--color-accent)' : 'var(--color-line)', color: tab === t.key ? 'var(--color-accent)' : 'var(--color-ink-soft)' }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <svg viewBox={`0 0 ${VB.w} ${VB.h}`} className="w-full" style={{ maxHeight: 240 }} role="img"
        ...
        {isLog ? (
          <>
            ...
            <text x={CONS.cx + 12} y={(CONS.cy + DLQ_LOG.cy) / 2} dominantBaseline="middle" style={{ fill: 'var(--color-muted)', fontFamily: 'var(--font-mono)', fontSize: 8.5 }}>
              consumer routes
            </text>
            <SvgNode n={SRC} title="events" sub="topic" color={LOG} state={lp === 0 ? 'active' : 'wait'} />
            <SvgNode n={CONS} title="consumer" sub="inspects + routes" color={LOG} state={lp === 1 ? 'active' : 'wait'} />
            <SvgNode n={DLQ_LOG} title="events.dlq" sub="dead-letter topic" color={LOG} state={lp === 2 ? 'active' : 'wait'} />
          </>
        ) : (
          <>
            <Edge from={rightOf(SRC)} to={leftOf(CONS)} on={onCons} />
            <Edge from={bottomOf(SRC)} to={topOf(DLQ_Q)} on={redrive} />
            <text x={SRC.cx + 12} y={(SRC.cy + DLQ_Q.cy) / 2} dominantBaseline="middle" style={{ fill: 'var(--color-muted)', fontFamily: 'var(--font-mono)', fontSize: 8.5 }}>
              broker redrives
            </text>
            <SvgNode n={SRC} title="queue" sub="main" color={QUEUE} state={qp === 0 ? 'active' : 'wait'} />
            <SvgNode n={CONS} title="consumer" sub={`receives ${rc}/${MAXRC}`} color={QUEUE} state={onCons ? 'active' : 'wait'} />
            <SvgNode n={DLQ_Q} title="DLQ" sub="dead-letter queue" color={QUEUE} state={redrive ? 'active' : 'wait'} />
          </>
        )}
      </svg>
      ...
      <p className="mt-2 min-h-[3.4rem] text-sm text-ink-soft">
        {isLog ? (
          ...
          <>
            The queue's <strong>broker</strong> redrives a message to its dead-letter queue on its own
            once it has been received and failed <code>maxReceiveCount</code> times, with no routing
